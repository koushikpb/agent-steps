...
- A hosted or shared code executor. The runner is a local `python3 -I` subprocess with a 10 s timeout, a 64 KiB output cap, CPU and file-size rlimits, sockets disabled, and `subprocess`/`ctypes`/`pip` imports blocked. It is a bounded guard, not a security sandbox. The script runs as your user and can read and write any file you can, and the network and subprocess blocks are Python-level guards against accidents, not enforcement. Live mode is off unless `ALLOW_LIVE=1`. Julius runs code in cloud containers with dedicated CPU and RAM (https://julius.ai/docs/get-started/containers); this demo does not.
